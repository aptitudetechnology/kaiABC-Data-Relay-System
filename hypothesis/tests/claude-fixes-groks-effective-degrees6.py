...
def _single_basin_volume_trial(N: int, K: float, worker_id: int = 0):
    ...
    # Scale evolution time with N to ensure convergence for larger systems
    steps = int(5000 * (N / 10))  # More steps for larger N
...
def test_phase_space_curvature_hypothesis_FIXED(N_values: List[int] = None,
    ...
    print("Step 1: Using coupling strength K scaled with N for reliable synchronization")
    print("-" * 50)
    K_margin = 2.0  # FIXED: Margin above K_c
    for N in N_values:
        K_c = 0.025 * np.sqrt(N / 10.0)  # FIXED: K_c increases with N
        results['K_c_values'].append(K_c)
        print(f"  N={N}: K_c ≈ {K_c:.4f} (increasing with N)")
    ...
    for i, N in enumerate(N_values):
        K = K_margin * results['K_c_values'][i]  # FIXED: Scale K with K_c
...
def test_critical_slowing_hypothesis(N_values: List[int] = None, trials_per_N: int = 100) -> Dict[str, Any]:
    ...
    for N in N_values:
        K_c_N = base_K_c * np.sqrt(N / 10.0)  # FIXED: K_c increases with N
...
def test_collective_mode_hypothesis(N_values: List[int] = None, trials_per_N: int = 100) -> Dict[str, Any]:
    ...
    for N in N_values:
        K_c_N = base_K_c * np.sqrt(N / 10.0)  # FIXED: K_c increases with N
        K_test = 0.08  # FIXED: Use fixed K that ensures synchronization
...
def test_finite_size_hypothesis(N_values: List[int] = None, trials_per_N: int = 100) -> Dict[str, Any]:
    ...
    for N in N_values:
        K_c_N = base_K_c * np.sqrt(N / 10.0)  # FIXED: K_c increases with N
        K = 0.08  # FIXED: Use fixed K that ensures synchronization
