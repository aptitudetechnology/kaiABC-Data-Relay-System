...
def test_phase_space_curvature_hypothesis_FIXED(N_values: List[int] = None,
    ...
    print("Step 1: Using fixed coupling strength K for reliable synchronization")
    print("-" * 50)
    K_fixed = 0.08  # FIXED: Use fixed K that ensures synchronization for all N
    for N in N_values:
        K_c = 0.025  # Constant K_c for all N (FIXED: was scaling incorrectly)
        results['K_c_values'].append(K_c)
        print(f"  N={N}: K_c ≈ {K_c:.4f} (constant estimate), using K = {K_fixed:.3f}")
    ...
    for i, N in enumerate(N_values):
        K = K_fixed  # FIXED: Use fixed K instead of scaled
...
def test_critical_slowing_hypothesis(N_values: List[int] = None, trials_per_N: int = 100) -> Dict[str, Any]:
    ...
    for N in N_values:
        K_c_N = base_K_c  # FIXED: Constant K_c for all N
